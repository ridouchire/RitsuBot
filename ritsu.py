...
from ritsu_config import *

require_level_string = {
  LEVEL_IGNORED         : '',
  LEVEL_DEVOICED_GUEST  : '',
  LEVEL_GUEST           : '',
  LEVEL_DEVOICED_MEMBER : u'You must be a member to access this command.',
  LEVEL_MEMBER          : u'Эта команда только для члена.',
  LEVEL_MODERATOR       : u'Эта команда только для модератора.',
  LEVEL_ADMIN           : u'Эта команда только для администратора',
  LEVEL_OWNER           : u'Эта команда только для владельца конференции.',
  LEVEL_BOT_OWNER       : u'Эта команда только для владельца бота',
}

access_level_string = {
  LEVEL_IGNORED         : u'игнорируемый',
  LEVEL_DEVOICED_GUEST  : u'гость без голоса',
  LEVEL_GUEST           : u'гость',
  LEVEL_DEVOICED_MEMBER : u'член без голоса',
  LEVEL_MEMBER          : u'член',
  LEVEL_MODERATOR       : u'модератор',
  LEVEL_ADMIN           : u'администатор',
  LEVEL_OWNER           : u'владелец конференции',
  LEVEL_BOT_OWNER       : u'владелец бота',
}

class RitsuBot(object):
  def __init__(self, configuration):
    self.version = '1.0.2-fucking-bot'
  ...
  def auto_reload_plugin(self, room, nick, access_level, parameters, message):
    self.unload_plugins()
    self.load_plugins()
    self.client.send(self.presence_room_bot(room, 'chat', ''))
    return u'Все плагины перезагружены, ошибок не обнаружено.'

  def admin_work(self, room, nick, access_level, parameters, message):
    self.unload_plugins()
    self.client.send(self.presence_room_bot(room, 'xa', 'Technical works.'))
  ...
  def load_plugin(self, plugin):
    ...
    try:
      p = __import__(plugin)
      reload(p) # FIXME: костыль, но работает
  ...
  def inter_unload_plugin(self, room, nick, access_level, parameters, message):
    self.unload_plugin(parameters)
    return 'Plugin %s unload!' % (parameters)

  def inter_load_plugin(self, room, nick, access_level, parameters, message):
    self.load_plugin(parameters)
    return 'Plugin %s load!' % (parameters)
  ...
  def presence_room_bot(self, room, show='available', status=""):
    ...
    c.setAttr('node', 'magnet2-fork.py')
  ...
  def got_iq(self, sess, iq):
    ...
    if iq.getQueryNS() == xmpp.NS_VERSION:
      ...
      reply_query.setTagData('name', 'RitsuBot')
  ...
  def command_list(self, room, nick, access_level, parameters, message):
    ...
    output = u'Доступные команды %s: %s.'%(access_level_string[access_level], ', '.join(cando))
    self.send_room_message('%s'%(room), output)
    return ''

  def command_source(self, room, nick, access_level, parameters, message):
    return '/me is RitsuBot %s.'%(self.version)

  def shutdown(self, quit_message="Shutting down..."):
...
if __name__ == "__main__":
  ...
  the_bot = RitsuBot(configuration1)
  the_bot.run()
